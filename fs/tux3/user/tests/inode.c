...
static void test02(struct sb *sb)
{
	...
	inode1 = tux_create_specific_inode(sb, sb->rootdir, 0x1000, iattr);
	test_assert(!IS_ERR(inode1));
	test_assert(is_defer_alloc_inum(inode1));
	unlock_new_inode(inode1);
	inode2 = tux_create_specific_inode(sb, sb->rootdir, 0x1000, iattr);
	...
	inode3 = tux_create_specific_inode(sb, sb->rootdir, 0x1000, iattr);
	test_assert(!IS_ERR(inode3));
	test_assert(is_defer_alloc_inum(inode3));
	unlock_new_inode(inode3);
	/* Try to alloc so far inum */
	inode4 = tux_create_specific_inode(sb, sb->rootdir, 0x10000000, iattr);
